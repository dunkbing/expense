...
export const extractExpensesData = (data: expenses[], locale: string) => {
...
export const extractTopExpenseCategoryData = (data: expenses[]): Array<{ name: string; value: number }> => {
	const dataMap: Record<string, { name: string; value: number }> = data.reduce((acc, datum) => {
		const obj = {
